...
namespace MicroAssistant.DataAccess
{
    public class ProProductonDetailAccessor
    {
        ...
        private ProProductonDetailAccessor()
        {
            ...
            cmdLoadProProductonDetail = new MySqlCommand(@"select p_d_id,price,p_num,p_code,create_time,user_id,pid from pro_producton_detail where (@PDId=0 or p_d_id=@PDId) and (@Price=0 or Price=@Price) and (@PNum=0 or PNum=@PNum) and (@PCode=0 or PCode=@PCode) and (@UserId=0 or UserId=@UserId) and (@PId=0 or PId=@PId ) {0} limit @PageIndex,@PageSize");
          ...
          //  cmdLoadProProductonDetail.Parameters.Add("@CreateTime", MySqlDbType.DateTime);
            ...
            cmdLoadProProductonDetailCount = new MySqlCommand("select count(*) from pro_producton_detail where (@PDId=0 or p_d_id=@PDId) and (@Price=0 or Price=@Price) and (@PNum=0 or PNum=@PNum) and (@PCode=0 or PCode=@PCode) and (@UserId=0 or UserId=@UserId) and (@PId=0 or PId=@PId) {0}");
